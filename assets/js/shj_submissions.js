  ...
  $(".btn").click(function() {
    ...
    var view_code_request = $.ajax({
      ...
      success: function(data) {
        if (type == "code") data.text = shj.html_encode(data.text);
        $(".modal_inside").html(
          '<pre class="code-column">' + data.text + "</pre>"
        );
        $(".modal_inside").prepend(
          "<p><code>" +
            data.file_name +
            " | Submit ID: " +
            row.data("s") +
        ...
        } else $("pre.code-column").addClass("shj_code");
      }
    });
    if (!shj.modal_open) {
      shj.modal_open = true;
    ...
    $.ajax({
      ...
      success: function(response) {
        if (response.done) {
          row
            .children(".status")
            .html('<div class="btn pending" data-code="0">PENDING</div>');
          noty({
            text: "Rejudge in progress",
            layout: "bottomRight",
            type: "success",
            timeout: 2500
          });
      ...
      success: function(response) {
        if (response.done) {
          $(
            ...
            .removeClass("fa-check-circle-o color11")
          ...
          $(
            ...
            .addClass("fa-check-circle-o color11");
        } else shj.loading_failed(response.message);
      }
    });
  });
...
$(document).ready(function() {
  function checkForPending() {
    ...
    $("tr.sub-info").each(function() {
      var text = $(this)
        .find("td.status div:first-child")
        .text();
      ...
      success: function(result) {
        if (result.status == "PENDING") {
          ...
          if (result.status == "SCORE") {
            ...
            var final_score = Math.ceil(
              (result.pre_score * parseInt(result.coefficient)) / 10000
            );
